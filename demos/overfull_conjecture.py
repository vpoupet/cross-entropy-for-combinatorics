...
N = 14  # number of vertices in the graph. Only used in the reward function, not directly relevant to the algorithm
...
percentile = 91  # top 100-X percentile we are learning from
super_percentile = 93  # top 100-X percentile that survives to next iteration
...
def calcScore(state):
    ...
    # Calculate the fractional edge chromatic number
    chi = G.chromatic_index()
    if chi == delta:
        return -INF
    chif = G.fractional_chromatic_index()
